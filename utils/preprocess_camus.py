...
def parse_args():
    ...
    parser.add_argument('-i', '--input_dir', type=str, default='/home/zy/coding/US_dataset/Camus/CAMUS_public/database_nifti')
    parser.add_argument('-o', '--output_dir', type=str, default='/home/zy/coding/sam2/sam2/us_dataset/camus')
    parser.add_argument('-f', '--split_file', type=str)
    args = parser.parse_args()

    return args
def camus_label_to_color(mask):
    color_map = np.zeros((*mask.shape, 3), dtype=np.uint8)
    color_map[mask == 1] = [0, 0, 255]   # LV cavity
    color_map[mask == 2] = [0, 255, 0]   # Myocardium
    color_map[mask == 3] = [255, 0, 0]   # LA cavity
    return color_map
...
def preprocess_data(input_path, output_path, split_file):
    ...
    for idx, patient_name in enumerate(patient_name_list):
        ...
        for view in ['2CH','4CH']:
            ...
            assert seq_info['spacing'] == seq_gt_info['spacing'] #spacing 把像素距离转成真实物理距离
            ...
            # ====== 可视化保存 ======
            # 创建 vis 文件夹
            vis_video_path = os.path.join(output_path, 'vis/video',seq_name)
            vis_mask_path = os.path.join(output_path, 'vis/mask',seq_name)
            os.makedirs(vis_video_path, exist_ok=True)
            ...
            for f in range(seq.shape[1]):
                # 原始帧
                frame_img = seq[:, f, :, :].transpose(1, 2, 0).astype(np.uint8)

                # 提取 mask 各结构（根据 CAMUS 标签定义）
                # label 1: LV cavity (蓝色)
                # label 2: Myocardium (绿色)
                # label 3: LA cavity (红色)
                mask_lv = (seq_gt[f] == 1).astype(np.uint8)
                mask_myo = (seq_gt[f] == 2).astype(np.uint8)
                mask_la = (seq_gt[f] == 3).astype(np.uint8)

                # 彩色 mask
                mask_color = np.zeros_like(frame_img)
                mask_color[mask_lv == 1] = [0, 0, 255]     # Blue for LV cavity
                mask_color[mask_myo == 1] = [0, 255, 0]    # Green for Myocardium
                mask_color[mask_la == 1] = [255, 0, 0]     # Red for LA cavity

                # 叠加彩色 mask 到原图
                blended = cv2.addWeighted(frame_img, 0.7, mask_color, 0.3, 0)

                frames.append(blended)
                frames_mask.append(mask_color)

            # 保存成帧序列
            for f, frame_img in enumerate(frames):
                cv2.imwrite(os.path.join(vis_video_path, f"{patient_name}_{view}_{f}.png"), frames[f])
                cv2.imwrite(os.path.join(vis_mask_path, f"{patient_name}_{view}_{f}_mask.png"), frames_mask[f])
