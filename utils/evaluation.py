...
import time
import pandas as pd
from utils.tools import corr, bias, std, bias_and_std, mae
...
def eval_camus(valloader, model, criterion, opt, args):
    ...
    gt_efs = {}
    sum_time = 0.0
    save_root = os.path.join("results_test_by_patient", args.task)
    os.makedirs(save_root, exist_ok=True)
    
    for batch_idx, (datapack) in enumerate(valloader):
        if batch_idx < 12 or batch_idx > 13 : continue # 只测试407
        imgs = Variable(datapack['image'].to(dtype = torch.float32, device=opt.device))
    ...
    ### 把t并入batch
        ...
        if pt is not None:
            pt_coords, pt_labels = pt
            pt_coords = pt_coords.view(B * T, pt_coords.shape[2], 2)
            pt_labels = pt_labels.view(B * T, pt_labels.shape[2])
        ...
        # semi
        # opt.semi = True
        val_loss = criterion(pred[:,0:1], masks)

    ### 把t并入batch
        imgs = imgs.view(B, T, C, H, W)
        pred = pred.view(B, T, 1, H, W)
    ...
    # --- 新增的闭运算代码开始 ---
        ...
        # smooth_dt_mask = np.zeros_like(seg_mask)

        # for t in range(seg_mask.shape[1]):
        #     dt = distance_transform_edt(seg_mask[0, t]) - distance_transform_edt(1 - seg_mask[0, t])
        #     dt = gaussian_filter(dt, sigma=3)   # sigma 越大越 smooth
        #     smooth_dt_mask[0, t] = (dt > 0).astype(np.uint8)
        # seg_mask = smooth_dt_mask 
    # --- 新增的闭运算代码结束 ---
        # seg_mask = gt #测试gt的积分参数
        # seg = gt
    # --------------------------
        ...
        for j in range(0, b):
            ...
            for idx, frame_i in enumerate(range(0,t)):
                ...
                #-------------------------保存图片----------------------------
                # 文件名：patient0430_4CH_f000_pred.png / _gt.png
                frame_tag = f"f{frame_i:03d}"
                ...
                pred_path = os.path.join(
                    output_dir,
                    f"{patient_name}_{view}_{frame_tag}_pred.png"
                )
                
                os.makedirs(output_dir, exist_ok=True)
                gt_path = os.path.join(
                    output_dir,
                    f"{patient_name}_{view}_{frame_tag}_gt.png"
                )

                # pred_i / gt_i 是 (1, H, W) → 取 [0] 变成 (H, W)
                cv2.imwrite(pred_path, pred_i[0])
                cv2.imwrite(gt_path, gt_i[0])
                # ================== 生成一帧 overlay 图用于写入 MP4 ==================
                img_tensor = imgs[j, frame_i]            # (C, H, W), on GPU
                ...
                gt_overlay[gt_mask] = (
                    (1 - alpha) * gt_overlay[gt_mask] + alpha * gt_color
                ).astype(np.uint8)
                ...
                pred_overlay[pred_mask] = (
                    (1 - alpha) * pred_overlay[pred_mask] + alpha * pred_color
                ).astype(np.uint8)

                # 横向拼：左 GT overlay | 右 Pred overlay  -> (H, 2W, 3)
                frame_concat = np.concatenate([gt_overlay, pred_overlay], axis=1)

                # OpenCV 用 BGR
                frame_bgr = cv2.cvtColor(frame_concat, cv2.COLOR_RGB2BGR)
                video_frames.append(frame_bgr)
                #---------------------------------------------------------------------
                # if (frame_i == 0) or (frame_i == t - 1):
                tp, fp, tn, fn = metrics.get_matrix(pred_i, gt_i)
                ...
                if opt.visual:
                    # f = mask_dict[patient_name][view]['ED']  == pred_i[0,...]
            ...
            if len(video_frames) > 0:
                h_video, w_video, _ = video_frames[0].shape
                fourcc = cv2.VideoWriter_fourcc(*'XVID')
                video_path = os.path.join(
                    output_dir,
                    f"{patient_name}_{view}.avi"   # 注意后缀改成 avi
                )
                video_fps = 30
                writer = cv2.VideoWriter(video_path, fourcc, video_fps, (w_video, h_video))
    ...
    if opt.mode == "train":
        ...
        if args.compute_ef:
            ...
            for patient_name in mask_dict:
                ...
                # print(patient_name, "EDV:", edv, "ESV:", esv, "EF_pred:", ef, "EF_gt:", gt_efs[patient_name])
            ...
            mae_val = mae(gt_ef_array, pred_ef_array)
            bias, err_std = bias_and_std(gt_ef_array, pred_ef_array)
            r = corr(gt_ef_array, pred_ef_array)

            print("MAE:", mae_val)
            print("bias:", bias, "std:", err_std, "corr:", r)
            # print(
...
def eval_camus_video(valloader, model, criterion, opt, args):
    model.eval()
    val_losses, mean_dice = 0, 0
    dices = np.zeros(opt.classes)
    tps, fps, tns, fns, hds, assds= [],[],[],[],[],[]
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        # if batch_idx > 1: continue # 只测试407//
        imgs = Variable(datapack['image'].to(dtype = torch.float32, device=opt.device))
        masks = Variable(datapack['label'].to(dtype = torch.float32, device=opt.device))
        spcaing = datapack['spacing'].detach().cpu().numpy()[0,:2][::-1] # remove z and reverse (y,x)
        # video to image
    ...
    # --------------------------
        ...
        for j in range(0, b):
            video_frames = []
            for idx, frame_i in enumerate(range(0,t)):
                if opt.semi and (frame_i > 0 and frame_i < t-1):
                    continue
                pred_i = np.zeros((1, h, w))
                pred_i[seg[j:j+1, frame_i,:, :] == 1] = 255
                gt_i = np.zeros((1, h, w))
...
def get_eval(valloader, model, criterion, opt, args):
    ...
    elif opt.eval_mode == "camus_video":  
        return eval_camus_video(valloader, model, criterion, opt, args)
    else:
