...
class PositionEmbeddingSine(nn.Module):# 对一个 H×W 的网格生成位置编码
    """
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    """
    Expand freqs_cis to be broadcastable to x.

    freqs_cis can optionally have leading dimensions (e.g., batch),
    but its last two dimensions must match x's last two.
    """
    assert freqs_cis.shape[-2:] == (x.shape[-2], x.shape[-1])
    leading_dims = x.ndim - 2
    extra = freqs_cis.shape[:-2]
    if len(extra) > leading_dims:
        raise ValueError(
            f"freqs_cis leading dims {extra} longer than target {leading_dims}"
        )
    shape = list(extra) + [1] * (leading_dims - len(extra)) + list(freqs_cis.shape[-2:])
...
def apply_rotary_enc(
    ...
    freqs_cis_k = freqs_cis_q if freqs_cis_k is None else freqs_cis_k
    if repeat_freqs_k and freqs_cis_k.shape[-2] != xk_.shape[-2]:
        r = xk_.shape[-2] // freqs_cis_k.shape[-2]
        freqs_cis_k = freqs_cis_k.repeat_interleave(r, dim=-2)
