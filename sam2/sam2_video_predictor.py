...
class SAM2VideoPredictor(SAM2Base):
    ...
    def __init__(
        ...
        # 当你在某一帧上新加点 / 新加 mask 时，这一帧本身通常会进入 cond_frame_outputs（条件帧，写入 memory）。
        # “non-cond memory” 指的是其他普通帧上存的记忆（non_cond_frame_outputs）。
    ...
    def add_new_points_or_box(
        ...
        points=None,
        labels=None,
        clear_old_points=True, # 清除当前帧point
        ...
        # 是否把当前帧当作 conditioning frame，者配置里 add_all_frames_to_correct_as_cond=True 时，所有被点过/修改过的帧都算 cond。
        is_cond = is_init_cond_frame or self.add_all_frames_to_correct_as_cond
        storage_key = "cond_frame_outputs" if is_cond else "non_cond_frame_outputs"

        # 获取这个对象在这一帧上之前预测的 mask logits，然后把它和新的点击一起喂给 SAM 的 mask decoder。
    ...
    def _consolidate_temp_output_across_obj(
        ...
        在某一帧 frame_idx 上，把 所有对象 的临时输出（temp_output_dict_per_obj）合并成一个跨对象的统一输出结构
        ...
        for obj_idx in range(batch_size):
            ...
            # 如果对象在当前帧的临时输出里找不到，就退一步去正式输出 output_dict_per_obj 里找以前的结果：
            # 先查 cond_frame_outputs[frame_idx]
            # 再查 non_cond_frame_outputs[frame_idx]
            if out is None:
    ...
    def propagate_in_video_preflight(self, inference_state):
        ...
        # 把“临时输出”变成“正式输出”，并补 memory 特征
        for obj_idx in range(batch_size):
            obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
            obj_temp_output_dict = inference_state["temp_output_dict_per_obj"][obj_idx]
            for is_cond in [False, True]: # 条件 / 非条件输出分开处理
            ...
            # 保证 cond / non-cond 不冲突
            for frame_idx in obj_output_dict["cond_frame_outputs"]:
                obj_output_dict["non_cond_frame_outputs"].pop(frame_idx, None)
                
    # reverse 为 True 的时刻： 当你希望模型从“当前起始帧”倒序推理直到视频开头（Frame 0）时，你在调用 propagate_in_video 时手动将其设为 True。
    @torch.inference_mode() # 视频传播
    def propagate_in_video(
        ...
        # 如果用户没指定起始帧，那就自动从所有有交互（有点/框/掩码输入）的帧里，找一个时间上最早的那一帧开始传播。
    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        expanded_image = image.expand(batch_size, -1, -1, -1)# 这是一种由 PyTorch 提供的零拷贝（Zero-copy）内存视图操作。它在逻辑上把 1 张图变成了 $N$ 张图，但物理内存中只有一份数据。
